...
import Detailed from './DetailCategory';
import {
  getAllCategories,
  removeStoreCategory,
  setHeaderTitle,
...
const ListUsers = () => {
  ...
  const { loading, error, categories, currentPage, totalItems, totalPages } = useSelector(
  ...
  useEffect(() => {
    dispatch(setHeaderTitle(['Manage Category']));
  }, [dispatch]);
  useEffect(() => {
  ...
  useEffect(() => {
    dispatch(getAllCategories({ ...condition, ...sortCriteria }));
    if (
      searchText ||
  ...
  const renderTableHeaders = () =>
    categoryTabHeader.map((tableHeader) => {
  ...
  const renderUserList = () => {
    ...
    const userList = categories.filter((item) => item.id !== userData.user.id);
    // if (newUser) {
    ...
    return (
      <tbody>
        {userList.map((category) => (
          <tr className="row-data" key={category.id}>
            <Popup modal trigger={<td className="popup-button">{category.id}</td>}>
              <Detailed isShowed currentUser={category} />
            </Popup>
            <Popup modal trigger={<td className="popup-button">{category.name}</td>}>
              <Detailed isShowed currentUser={category} />
            </Popup>
            <td className="edit-delete">
              <Link to={`/categories/edit/${category.id}`} className="btn btn-light ">
  ...
  return (
    <div id="user-list-table">
      ...
      <Row>
        ...
        <Col md="3">
          <Link to="/categories/create" className="btn btn-primary btn-block">
            Create new category
          </Link>
        </Col>
      </Row>
